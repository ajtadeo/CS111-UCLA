...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count    = 0; // blocks that cannot be written to
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;
	superblock.s_free_inodes_count = NUM_FREE_INODES;
	superblock.s_first_data_block  = SUPERBLOCK_BLOCKNO; // location of the super block
	superblock.s_log_block_size    = 0; /* size of block as a power of 2: 2^0 = 1024 */
	superblock.s_log_frag_size     = 0; /* size of frag as a power of 2: 2^0 = 1024 */
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_block[0] = "Hell";
	hello_world_inode.i_block[1] = "o wo";
	hello_world_inode.i_block[2] = "rld";
