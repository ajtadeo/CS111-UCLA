...
#define FIRST_DATA_BLOCK 0 /* larger than 1 KB = 0 */
#define LOG_BLOCK_SIZE 0 /* size of block as a power of 2: 2^0 = 1024 */
#define LOG_FRAG_SIZE 0 /* size of frag as a power of 2: 2^0 = 1024 */
#define INIT_MNT_COUNT 0
...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count    = R_BLOCKS_COUNT; // blocks that cannot be written to
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;
	superblock.s_free_inodes_count = NUM_FREE_INODES;
	superblock.s_first_data_block  = FIRST_DATA_BLOCK; 
	superblock.s_log_block_size    = LOG_BLOCK_SIZE; 
	superblock.s_log_frag_size     = LOG_FRAG_SIZE; 
	superblock.s_blocks_per_group  = BLOCKS_PER_GROUP; // one group in the fs
	superblock.s_frags_per_group   = 0; // zeroed out
	...
	superblock.s_def_resuid        = ROOT_UID; /* root */
	superblock.s_def_resgid        = ROOT_GID; /* root */
