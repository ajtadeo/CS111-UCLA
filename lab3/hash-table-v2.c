...
void hash_table_v2_add_entry(struct hash_table_v2 *hash_table,
                             ...
                             uint32_t value)
{
	// only initialize once due to static definition
	static pthread_mutex_t mutex_write = PTHREAD_MUTEX_INITIALIZER;
	static pthread_mutex_t mutex_read = PTHREAD_MUTEX_INITIALIZER;
	int err = 0;
	
	err = pthread_mutex_lock(&mutex_read); if (err != 0){ exit(errno); }					// read lock
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	err = pthread_mutex_unlock(&mutex_read); if(err != 0){ exit(errno); }					// read unlock

	err = pthread_mutex_lock(&mutex_write); if (err != 0){ exit(errno); }					// write lock
	...
	err = pthread_mutex_unlock(&mutex_write); if (err != 0){ exit(errno); }				// write unlock
